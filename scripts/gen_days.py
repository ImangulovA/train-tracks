...
def load_existing(out):
    ...
    txt = open(out).read()
    existing = {}
    # Each entry is: "IDX": { ... },  (object may contain nested [] but no {}).
...
def main():
    ...
    for idx in range(lo, hi + 1):
        ...
        print(f"day {idx:>3}  {wd}  {tier:<8} {size}x{size}  givens={len(blob['givens'])}  track={len(blob['solution'])}")
